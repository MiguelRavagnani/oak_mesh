use crate::mesh::utils::{from_bcc_vector3, quat_shortest_arc, rand_bcc, triangle_area};
...
impl MeshFactory {
    ...
    fn create_grass_blade(&self) -> Gd<ArrayMesh> {
        ...
        let uvs = PackedVector2Array::from(&[
            Vector2::new(0.0, 0.0),
            Vector2::new(1.0, 0.0),
            Vector2::new(0.0, 0.33),
            Vector2::new(1.0, 0.33),
            Vector2::new(0.0, 0.86),
            Vector2::new(1.0, 0.86),
            Vector2::new(0.5, 1.0),
        ]);
    ...
    fn generate_grass_distribution(
        ...
        godot_print!(
            "[Rust] generate_grass_distribution called with density: {}",
            density
        );
        ...
        for index in (0..indices.len()).step_by(3) {
            ...
            for _ in 0..blades_per_face {
                ...
                let q1 = Quaternion::from_axis_angle(
                    Vector3::UP,
                    (randf_range(0.0, 360.0) * std::f64::consts::PI / 180.0) as f32,
                );
                ...
                let params = Color {
                    ...
                    b: (randf_range(sway_pitch.x as f64, sway_pitch.y as f64)
                        * std::f64::consts::PI
                        / 180.0) as f32,
                    a: (randf_range(sway_yaw.x as f64, sway_yaw.y as f64) * std::f64::consts::PI
                        / 180.0) as f32,
                };
    ...
    fn generate_grass_with_rgb_weights(
        ...
        for index in (0..indices.len()).step_by(3) {
            ...
            for _ in 0..blades_per_face {
                ...
                // Sample RGB weights from single texture
                let blade_color =
                    self.sample_rgb_weights(uv, weight_image, x_color, y_color, z_color);
    ...
    fn generate_grass_blades_internal(
        ...
        godot_print!(
            "[Rust] Mesh data - Vertices: {}, Normals: {}, Indices: {}",
            vec_positions.len(),
            vec_normals.len(),
            vec_indices.len()
        );
        ...
        if indices.is_empty() {
                ...
                for _ in 0..blades_per_face {
                    ...
                    let params = Color {
                        ...
                        b: (randf_range(sway_pitch.x as f64, sway_pitch.y as f64)
                            * std::f64::consts::PI
                            / 180.0) as f32,
                        a: (randf_range(sway_yaw.x as f64, sway_yaw.y as f64)
                            * std::f64::consts::PI
                            / 180.0) as f32,
                    };
            ...
            for index in (0..indices.len()).step_by(3) {
                ...
                for _ in 0..blades_per_face {
                    ...
                    let q1 = Quaternion::from_axis_angle(
                        Vector3::UP,
                        (randf_range(0.0_f64, 360.0_f64) * std::f64::consts::PI / 180.0) as f32,
                    ); // Fixed: removed _f64 and used proper conversion
                    let q2 = quat_shortest_arc(Vector3::UP, normal);
                    ...
                    let params = Color {
                        ...
                        b: (randf_range(sway_pitch.x.as_f64(), sway_pitch.y.as_f64())
                            * std::f64::consts::PI
                            / 180.0) as f32, // Fixed: proper degree to radian conversion
                        a: (randf_range(sway_yaw.x.as_f64(), sway_yaw.y.as_f64())
                            * std::f64::consts::PI
                            / 180.0) as f32, // Fixed: proper degree to radian conversion
                    };
